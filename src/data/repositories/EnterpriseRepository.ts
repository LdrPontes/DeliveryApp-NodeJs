...
export class EnterpriseRepository implements IEnterpriseRepository {
    async save(name: string, document_type: number, document: string, logo_url: string, enterprise_id: number, category_id: number): Promise<Enterprise> {
        try {
    ...
    async update(id: number, name: string, logo_url: string, category_id: number): Promise<Enterprise> {
        try {

            const repository = getRepository(Enterprise);

            const result = await repository.update(id, { name: name, logo_url: logo_url, category_id: category_id })
